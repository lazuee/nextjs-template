...
const ToolbarBtn = (_props: Record<string, any>) => {
	let props = { ..._props };
	let className = "tw-flex tw-items-center tw-justify-center tw-w-10 tw-h-10 hover:tw-text-black dark:hover:tw-text-white";
...
export const Toolbar = () => {
	...
	useScrollPosition(({ currPos }) => {
		...
		setIsBottom(-currPos.y + window.outerHeight + offset >= document.getElementById("__next")!.scrollHeight);
	});
	useEffect(() => {
		setMounted(true);
	}, []);
	...
	return (
		<div className="tw-flex tw-flex-col tw-gap-y-2 sm:tw-flex-col-reverse">
			{showFull && (
				<div className={classNames("tw-grid tw-grid-flow-row", "tw-rounded-lg tw-border-2 tw-shadow-md tw-bg-bg dark:tw-bg-dark-bg")}>
					<ToolbarBtn
						aria-label="Toggle Dark Mode"
						onClick={() => {
							setTheme(theme === "dark" || resolvedTheme === "dark" ? "light" : "dark");
						}}>
						{mounted && (theme === "dark" || resolvedTheme === "dark") ? <HiSun className="tw-w-6 tw-h-6" /> : <HiMoon className="tw-w-6 tw-h-6" />}
					</ToolbarBtn>
